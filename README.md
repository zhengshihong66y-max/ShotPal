...
- 支持在设置页登录 Instagram、小红书和 YouTube，拉片宝会优先使用内部登录态读取收藏和下载内容，不再依赖后台打开 Chrome。
...
- 支持维护拉片宝内部账号登录态，并可一键清除内部 Cookie 和网页登录数据。
...
- **导入链路增强：** 网络导入状态、转码、下载器检查、`yt-dlp` 自修复、小红书解析、内部账号登录态、Instagram/小红书收藏候选刷新和终态保护都有更明确的状态处理。
